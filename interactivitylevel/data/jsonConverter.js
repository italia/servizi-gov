...
var jsonConverter = function jsonLd2JsonModel(pathUrl){
    ...
    //var data = fs.readFileSync('./data/InteractivityLevel.jsonld');
...
function fetchJsonToModel(dataGraph, pathUrl){
    for(var i = 0; i < dataGraph.length; i++){
        ...
        if(document["clvapit:hasRankOrder"] != null && document["clvapit:hasRankOrder"] != undefined && document["clvapit:hasRankOrder"] == "1" ){
            ...
            var itemIt = { 
                "idlevel": document["dcterms:identifier"],
                "rdfuri": document["@id"], 
                "label": labelIt,
...
function writeJsonResult(jsonToWrite){
    // fs.writeFile('../data/interactivityLevel.json', JSON.stringify(jsonToWrite), (err) => {
    ...
    //     return retVal;
    //   });
    fs.writeFileSync('./data/interactivityLevel.json', JSON.stringify(jsonToWrite));
}
